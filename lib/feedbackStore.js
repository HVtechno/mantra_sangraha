...
// Wipe all visit / user / seva counters back to zero (feedback is NOT touched).
async function resetStats() {
  if (useRedis) {
    const vk = (await redis(['KEYS', V_DAY + '*'])) || [];
    const uk = (await redis(['KEYS', U_DAY + '*'])) || [];
    const keys = [V_TOTAL, U_TOTAL, S_COUNT, S_SUM, ...vk, ...uk];
    if (keys.length) await redis(['DEL', ...keys]);
    return true;
  }
  try { require('fs').unlinkSync(statsFile()); } catch {}
  return true;
}
...
module.exports = { saveFeedback, listFeedback, archiveItem, restoreItem, deleteItem, pruneOldFeedback, backend, counts, bumpVisit, bumpSeva, getStats, resetStats };
