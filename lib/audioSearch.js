...
// archive.org asks callers to identify themselves — a descriptive UA keeps us a
// good citizen and off any rate-limit lists (unlike vignanam, archive is happy
// with a non-browser UA).
const UA =
  'MantraSangraha/1.0 (+https://mantra-sangraha.onrender.com; ad-free devotional mantra app) archive.org-audio-lookup';
...
function scoreDoc(nameNorm, doc) {
  ...
  if (BAD.test(text)) s -= 30; // strongly demote lectures/discourses/commentaries
...
async function find(name) {
  ...
  let docs;
  try { docs = await searchDocs(q, 40); } catch (e) { return { ok: false, error: 'search_failed', message: String(e.message || e) }; }
  ...
  // Resolve the top candidates, then choose deliberately: a real TRACK match (a
  // filename that IS this mantra) beats a stray track, and among real matches the
  // higher DOC score wins — so a clean recitation outranks a lecture/discourse
  // that merely shares the name (those are BAD-penalised at the doc level). The
  // old code picked purely by track match and discarded the doc score once it
  // began resolving, which is how a discourse could win.
  const resolved = [];
  for (const cand of ranked.slice(0, 6)) {
    try {
      const r = await resolveItem(cand.identifier, q);
      if (r) resolved.push({ r, score: cand.score, match: r.match || 0 });
    } catch { /* try next */ }
  }
  if (resolved.length) {
    const strong = resolved.filter((x) => x.match >= 0.5);
    const pool = strong.length ? strong : resolved;
    const cmp = strong.length
      ? (a, b) => (b.score - a.score) || (b.match - a.match)   // real tracks: best doc score
      : (a, b) => (b.match - a.match) || (b.score - a.score);  // fallback: best track match
    pool.sort(cmp);
    const out = { ...pool[0].r, alternatives };
    cache.set(key, out);
    return out;
  }
